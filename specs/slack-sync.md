...
Message shape (v2; emoji as **shortcodes** — Slack normalizes literal emoji in stored text, which would break read-back equality):

```
<https://github.com/postylem|postylem> starred <https://github.com/Open-Athena/Kelp|Open-Athena/Kelp> · 2026-07-28 16:01Z · 1,237 :star:
```

The event kind is carried by a **per-message avatar** (`chat.postMessage` `icon_url`, needs the `chat:write.customize` bot scope): `https://watchy.rbw.sh/icons/<slug>-<kind>.png`, where `<slug>` is the target's org lowercased (orgs with generated icons: `AVATAR_ORGS` in `scripts/gen-pfp.py`; others fall back to `gh`). Icons are org avatar + emoji badge (⭐/💔/📣/🔇), generated by `gen-pfp.py icons` into `www/public/icons/`, served by CFP. The trailing running total is the target's latest `counts` row (repo stars `:star:`, org followers `:mega:`); it's omitted when no count exists. v1 messages (leading kind emoji, no total) remain in-channel unedited — the reconcile is id-based, never content-based, so no migration.
