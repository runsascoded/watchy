...
## v10 — max-decay scoring + curated LI URLs + showcase v3 (RW, 2026-08-11)

- **Interest score recency = max-decay, not √Σ**: `max 2^(−age/hl)` over still-active events × `(1 + 0.15·(n−1))` multi-event bonus. The √Σ sum let event *count* outweigh fame — t11s (2 stars, 6/1) outranked norvig (1 star, 6/22, 2.5× followers) because log10 compresses fame gaps to ~11% while a second event added ~+25%. Max keeps "newest action" the dominant recency axis; the bonus still credits engagement depth (and still suppresses many-tiny-events accounts). Mirrors updated: `www` `scoreActor` (+ breakdown TT + column TT), `cfw` `interestScore` (`/api/actors/summary`), `tmp/details-input.py` (`top-actors-msg.py` marked superseded). Reshuffle: norvig #1; kashif/yzhangcs/nmntz out of top-10, bingzhengwei/brendanartley/GusSand in (recency tilt).
- **Migration 0013 `actors.li_url` + `li_company_url`**: curated LinkedIn profile/company URLs — blind LI searches are often junk ("Jerome Kelleher" people-search, "Big Data Institute" company-search full of impostors). Renderers prefer curated over blog-derived over search: `renderActorOp` LI link + company link, `weekly.ts` `affil`, www `liSearch`. Curated so far: norvig → `/in/pnorvig` + Stanford school URL, jeromekelleher → `/in/jeromekelleher` + `/company/bigdatainstitute`. Enrichment upsert doesn't touch the columns (curation survives refreshes). Durable auto-fill = the LLM-judged lookup (pending `ANTHROPIC_API_KEY`).
- **t11s msgs combined**: the un-enriched flat haliax msg (0 replies/reactions — checked via `tmp/check-msg.py` first) deleted; the marin details msg re-rendered with both events ("*t11s* ⭐ marin, haliax · 6/1"); `slack_posts` + `event-msg-ts.json` repointed to the surviving ts.
