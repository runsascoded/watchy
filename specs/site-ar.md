...
3. **Auth wall** — following the `$oa/marin-gcs-usage` precedent (gcs.oa.dev): CF Access app in the **Open Athena CF account**, edge-gating the hostname; app code stays coarse (no in-app JWT validation). applitrack's app-level OAuth+roles is more than we need.
   ...
   - **Dashboard runbook (RW — deploy token lacks zone + Zero Trust perms)**, OA CF account:
     ...
     2. ✅ Access app **watchy** (created 2026-08-07 via Claude-driven dashboard session): destinations `watchy.oa.dev`, `watchy-internal.pages.dev`, `*.watchy-internal.pages.dev` (wildcard covers per-deploy preview URLs); policy = reusable **"Openathena Whitelist"** (allow, Emails ending in `openathena.ai`, 24h session — same policy as oa-index; GCS usage's "OA domain + whitelist" is a legacy app-local policy, not attachable). All three surfaces verified 302 → `openathena-ai-pages.cloudflareaccess.com` login.
   - To grant external collaborators: edit the reusable "Openathena Whitelist" policy (note: shared with oa-index — changes affect both) or attach a second watchy-specific policy.
